...
export async function buildDispatchDebugReport(
  ...
  const profileDetails = await Promise.all(
    rows.map(async (p) => {
      ...
      const { data: listings } = await supabase
        .from("listings")
        .select("id,subcategory_kind,title_es,is_verified,seller_id")
        .in("seller_id", pool)
        .eq("is_verified", true);
  ...
  if (drivers.length === 0 && rows.length > 0) {
    checks.push(
      "Active drivers exist but dispatch filtered all out — fix listing link, colonias, or clear busy ride_bookings.",
    );
  }
  if (drivers.length > 0) {
  ...
  return {
    ...
    drivers,
    profiles: profileDetails.map(({ serves_pickup: _s, ...rest }) => rest),
    checks,
  };
}
