...
import { plumbingStarterMenu } from "@/lib/listing-service-menu";
import { PLUMBING_SERVICE } from "@/lib/provider-services";
...
> = {
  es: {
    navHome: "← Inicio",
    heroEyebrow: "Para plomeros y fontaneros a domicilio",
    heroTitle: "Consigue más clientes de plomería en San Miguel.",
    ...
    ctaPrimary: "Registrarme como plomero",
    ...
    whySub: "Tres razones para publicar tu servicio de plomería en SMA.",
    why1Title: "Clientes que buscan plomero de verdad",
    why1Body: "Familias y expatriados te encuentran cuando buscan fugas, inodoros, drenajes o calentadores en su colonia.",
    ...
    how2Body: "Plantilla con 16 trabajos típicos (grifos, inodoros, destapes, calentadores, etc.). Ajustas precios a tu zona.",
    ...
    buyerEyebrow: "¿Buscas plomero?",
    buyerTitle: "Encuentra plomeros cerca de ti.",
    buyerSub: "Plomeros verificados en San Miguel con menú y cotización en la app.",
    buyerCta: "Ver plomeros en SMA",
  ...
  en: {
    navHome: "← Home",
    heroEyebrow: "For plumbers and mobile plumbing",
    heroTitle: "Get more plumbing clients in San Miguel.",
    ...
    ctaPrimary: "Sign up as a plumber",
    ctaSecondary: "See how it works",
    badgeFree: "Free signup",
    badgeVerified: "Verified profile",
    badgeReach: "Expats & local families",
    whyTitle: "Why join?",
    whySub: "Three reasons to list your plumbing service in SMA.",
    why1Title: "Customers searching for real plumbers",
    why1Body: "Families and expats find you for leaks, toilets, drains, and water heaters in their neighborhood.",
    ...
    how2Body: "Template with 16 typical jobs (faucets, toilets, drains, heaters, etc.). Adjust for your area.",
    ...
    buyerEyebrow: "Need a plumber?",
    buyerTitle: "Find plumbers near you.",
    buyerSub: "Verified plumbers in San Miguel with in-app quotes.",
    buyerCta: "Browse plumbers in SMA",
...
export function generateMetadata({
  ...
  const title =
    lang === "es"
      ? "Plomería en San Miguel de Allende | Naranjogo"
      : "Plumbing services in San Miguel de Allende | Naranjogo";
  return {
    title,
...
export default function PlomeriaLandingPage({
  searchParams,
}: {
  searchParams?: { lang?: string };
}) {
  const lang = langFromParam(searchParams?.lang);
  const t = COPY[lang];
  const menu = plumbingStarterMenu();
  ...
  const signupHref = `/unete?service=${PLUMBING_SERVICE}&lang=${lang}`;
  const browseHref = `/?category=services&q=${encodeURIComponent(
    lang === "es" ? "plomero" : "plumber",
  )}&lang=${lang}`;
