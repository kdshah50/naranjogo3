...
resp AS (
  ...
  INNER JOIN LATERAL (
    ...
    WHERE m.conversation_id = c.id AND m.sender_id::text = c.buyer_id::text
  ) bm ON bm.first_buyer IS NOT NULL
  INNER JOIN LATERAL (
    SELECT MIN(m.created_at) AS first_seller
    FROM public.listing_messages m
    WHERE m.conversation_id = c.id
      AND m.sender_id::text = c.seller_id::text
      AND m.created_at >= bm.first_buyer
  ) sm ON sm.first_seller IS NOT NULL
  WHERE EXISTS (SELECT 1 FROM ids i0 WHERE i0.listing_id = c.listing_id::text)
...
seller_pen AS (
  ...
  LEFT JOIN public.users u ON u.id::text = l.seller_id::text
