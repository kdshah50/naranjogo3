/**
 * Re-send buyer WhatsApp for scheduled / in_progress lifecycle phase.
 *
 * Usage:
 *   npx tsx scripts/retry-buyer-phase-notify.ts NG-00FDB90F scheduled
 *   npx tsx scripts/retry-buyer-phase-notify.ts <booking-uuid> in_progress --force
 *
 * --force clears prior buyer_whatsapp_phase event so notify can run again.
 */
...
async function main() {
  ...
  if (!ref || (phase !== "scheduled" && phase !== "in_progress")) {
    console.error(
      "Usage: npx tsx scripts/retry-buyer-phase-notify.ts <ticket-or-uuid> [scheduled|in_progress] [--force]",
    );
    process.exit(1);
  }
  ...
  const { data: booking, error } = isUuid
    ? await supabase
        ...
        .select("id,ticket_code,status,payment_status,buyer_id")
        .eq("id", ref)
        .maybeSingle()
    : await supabase
        .from("service_bookings")
  ...
  console.log("\n=== Retry buyer lifecycle WhatsApp ===\n");
  ...
  console.log("buyer WhatsApp digits:", buyerDigits || "(none)");
  ...
  if (force) {
    await supabase
      ...
      .eq("to_status", phase);
    console.log("\n--force: cleared buyer_whatsapp_phase dedupe event.");
  }

  const result = await notifyBuyerLifecyclePhase(supabase, String(booking.id), phase);
  console.log("\nResult:", result);
}
